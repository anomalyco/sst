...
var SST_STREAMING = os.Getenv("SST_STREAMING") == "true"
		...
		for {
			...
			case msg := <-client.Read():
				...
				if msg.Type == bridge.MessageResponse && msg.ID == requestID {
					if SST_STREAMING {
						req, _ := http.NewRequest("POST", "http://"+LAMBDA_RUNTIME_API+"/2018-06-01/runtime/invocation/"+requestID+"/response", msg.Body)
