...
interface FunctionEventInvokeConfigArgs {
  ...
  maxAge?: Input<Duration>;
  /**
   * The destination for successful invocations. Can be a Queue, Topic, Function, or ARN.
   * @example
   * ```js
   ...
   * }
   * ```
   * Or pass an ARN directly:
   * ```js
   * {
   *   onSuccess: "arn:aws:sqs:us-east-1:123456789012:my-queue"
   * }
   * ```
   */
  onSuccess?: Input<Queue | SnsTopic | Function | FunctionArn | string>;
  /**
   * The destination for failed invocations. Can be a Queue, Topic, Function, or ARN.
   * @example
   * ```js
   * const myDlq = new sst.aws.Queue("MyDLQ");
   * // ...
   * {
   *   onFailure: myDlq
   * }
   ...
   * ```js
   * {
   *   onFailure: "arn:aws:sns:us-east-1:123456789012:my-topic"
   * }
   * ```
   */
  onFailure?: Input<Queue | SnsTopic | Function | FunctionArn | string>;
}
...
export interface FunctionArgs {
  ...
  /**
   * Configure asynchronous invocation settings for the function, such as retry attempts,
   * maximum event age, and destinations for success or failure.
   *
   * @example
   * Set a dead-letter queue for failed invocations:
   * ```js
   * const myDlq = new sst.aws.Queue("MyDLQ");
   * // ...
   * {
   ...
   * }
   * ```
   * Configure retries and success destination:
   * ```js
   * const successTopic = new sst.aws.SnsTopic("SuccessTopic");
   * // ...
   * {
   *   asyncInvoke: {
   *     retryAttempts: 1,
   *     onSuccess: successTopic
   *   }
   * }
   * ```
   */
  eventInvokeConfig?: Input<FunctionEventInvokeConfigArgs>;
  /**
  ...
  transform?: {
    ...
    /**
     * Transform the EventInvokeConfig resource.
     */
...
export class Function extends Component implements Link.Linkable {
  ...
  private eventInvokeConfig: Output<
    lambda.FunctionEventInvokeConfig | undefined
  >;
  ...
  constructor(
    ...
    const eventInvokeConfigArgs = normalizeEventInvokeConfig();
    ...
    const eventInvokeConfig = createAsyncInvokeConfig();
    createProvisioned();
    ...
    function normalizeEventInvokeConfig() {
      return output(args.eventInvokeConfig).apply((eventInvokeConfig) => {
        if (!eventInvokeConfig) return;
        return {
          maxAge: eventInvokeConfig.maxAge
            ? toSeconds(eventInvokeConfig.maxAge)
            : undefined,
          retryAttempts: eventInvokeConfig.retryAttempts,
    ...
    function getDestinationArn(
      dest: Input<
        Queue | SnsTopic | Function | FunctionArn | string | undefined
      >,
    ): Output<string | undefined> {
      return output(dest).apply((d) => {
        if (!d) return undefined;
        if (typeof d === "string") return d;
        if (d instanceof Queue) return d.arn.get();
        if (d instanceof SnsTopic) return d.arn.get();
        if (d instanceof Function) return d.arn.get();
        return undefined;
      });
    }

    function createAsyncInvokeConfig() {
      return all([eventInvokeConfigArgs, fn, role]).apply(([cfg, fn, role]) => {
        if (!cfg) return undefined;

        const onSuccessArn = getDestinationArn(cfg.onSuccess);
        const onFailureArn = getDestinationArn(cfg.onFailure);

        if (
          cfg.retryAttempts === undefined &&
          cfg.maxAge === undefined &&
          !cfg.onSuccess &&
          !cfg.onFailure
        ) {
          return undefined;
        ...
        return new lambda.FunctionEventInvokeConfig(
          ...transform(
            ...
            `${name}AsyncInvokeConfig`,
            {
              ...
              maximumRetryAttempts: cfg.retryAttempts,
              maximumEventAgeInSeconds: cfg.maxAge,
              destinationConfig: all([onSuccessArn, onFailureArn]).apply(
                ([onSuccessArn, onFailureArn]) => {
                  const onSuccess = onSuccessArn
                    ? { destination: onSuccessArn }
                    : undefined;
                  const onFailure = onFailureArn
