...
type ChangeDetector struct {
	// layoutDetector is used to detect project layout and dependencies
...
const (
	...
	ChangeTypeLayout         ChangeType = "layout"          // Project layout changed
...
func NewChangeDetector(config ChangeDetectorConfig) (*ChangeDetector, error) {
	if config.LayoutDetector == nil {
		return nil, fmt.Errorf("layout detector is required")
	}
...
func (cd *ChangeDetector) DetectChanges(functionID, handler string) (*ChangeResult, error) {
	...
	layout, err := cd.layoutDetector.DetectLayout(handler)
	if err != nil {
		result.HasChanges = true
		result.ChangeTypes = append(result.ChangeTypes, ChangeTypeLayout)
		result.Reason = fmt.Sprintf("Failed to detect project layout: %v", err)
		return result, nil
	}
	
	// Check if layout has changed
	if cd.hasLayoutChanged(cacheEntry, layout) {
		result.HasChanges = true
		result.ChangeTypes = append(result.ChangeTypes, ChangeTypeLayout)
		result.Reason = "Project layout has changed"
		return result, nil
	}
	...
	if !isValid {
		...
		changedFiles, changeTypes := cd.analyzeFileChanges(cacheEntry, layout)
		result.HasChanges = true
		result.ChangedFiles = changedFiles
		result.ChangeTypes = changeTypes
		result.Reason = cd.buildChangeReason(changedFiles, changeTypes)
		result.AffectedDependencies = cd.findAffectedDependencies(changedFiles, layout)
		return result, nil
	}
...
// hasLayoutChanged checks if the project layout has changed since the last build
func (cd *ChangeDetector) hasLayoutChanged(cacheEntry *CacheEntry, currentLayout *LayoutInfo) bool {
	if cacheEntry.LayoutInfo == nil {
		return false // No previous layout info, assume no change for now
	}
	
	cached := cacheEntry.LayoutInfo
	
	// Check key layout properties that would affect builds
	if cached.Type != currentLayout.Type {
		return true
	}
	
	if cached.WorkspaceDir != currentLayout.WorkspaceDir {
		return true
	}
	
	if cached.PackageName != currentLayout.PackageName {
		return true
	}
	
	if cached.SourceRoot != currentLayout.SourceRoot {
		return true
	}
	
	if cached.HasSrcDirectory != currentLayout.HasSrcDirectory {
		return true
	}
	
	// Only check Python paths if they're significantly different
	// Minor differences in path ordering shouldn't trigger rebuilds
	if len(cached.PythonPath) != len(currentLayout.PythonPath) {
		return true
	}
	
	// Create sets for comparison to ignore order
	cachedPaths := make(map[string]bool)
	for _, path := range cached.PythonPath {
		cachedPaths[path] = true
	}
	
	for _, path := range currentLayout.PythonPath {
		if !cachedPaths[path] {
			return true
		}
	}
...
func (cd *ChangeDetector) analyzeFileChanges(cacheEntry *CacheEntry, layout *LayoutInfo) ([]string, []ChangeType) {
	...
	newFiles := cd.findNewRelevantFiles(cacheEntry, layout)
...
func (cd *ChangeDetector) findNewRelevantFiles(cacheEntry *CacheEntry, layout *LayoutInfo) []string {
	var newFiles []string
	
	// Get all relevant files in the project
	relevantFiles, err := cd.findRelevantFiles(layout)
	if err != nil {
...
func (cd *ChangeDetector) findRelevantFiles(layout *LayoutInfo) ([]string, error) {
	var relevantFiles []string
	
	// Start from the workspace directory
	err := filepath.Walk(layout.WorkspaceDir, func(path string, info os.FileInfo, err error) error {
		if err != nil {
...
func (cd *ChangeDetector) buildChangeReason(changedFiles []string, changeTypes []ChangeType) string {
	...
	for _, changeType := range changeTypes {
		...
		case ChangeTypeLayout:
			reasons = append(reasons, "Project layout changed")
...
func (cd *ChangeDetector) findAffectedDependencies(changedFiles []string, layout *LayoutInfo) []string {
	var affected []string
...
func (cd *ChangeDetector) UpdateCacheAfterBuild(functionID, handler string, layout *LayoutInfo, buildOutput *CachedBuildOutput) error {
