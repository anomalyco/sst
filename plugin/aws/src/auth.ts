...
export interface AuthArgs {
   ...
   * the `issuer` function.
   */
  issuer?: sst.Input<string | FunctionArgs>;
  /**
 ...
 * ```
 */
export class Auth extends AWSComponent implements sst.Linkable {
  private readonly _table: Dynamo;
  private readonly _issuer: sst.Output<Function>;
  ...
  constructor(name: string, args: AuthArgs, opts?: sst.ComponentOptions) {
    ...
    self.registerVersion({
      ...
      message: [
        ...
        `  - Rename "Auth" to "Auth.v${sst.version[name]}". Learn more about versioning - https://sst.dev/docs/components/#versioning`,
      ].join("\n"),
    ...
    function createIssuer() {
      ...
      return functionBuilder(
        ...
        fn,
        {
          link: [table],
          environment: {
            OPENAUTH_STORAGE: sst.json.stringify({
              type: "dynamo",
