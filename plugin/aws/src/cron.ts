...
import { FunctionArgs, FunctionArn, Function } from "./function";
...
export interface CronArgs {
   ...
   * ```
   */
  job?: sst.Input<string | FunctionArgs | FunctionArn>;
  /**
   * The function that'll be executed when the cron job runs.
   *
   * @example
   ...
   * ```
   */
  event?: sst.Input<Record<string, sst.Input<string>>>;
  /**
   ...
   * ```
   */
  schedule: sst.Input<`rate(${string})` | `cron(${string})`>;
  /**
 ...
 * ```
 */
export class Cron extends AWSComponent {
  private name: string;
  private fn?: FunctionBuilder;
  private rule: cloudwatch.EventRule;
  private target: cloudwatch.EventTarget;

  constructor(name: string, args: CronArgs, opts?: sst.ComponentOptions) {
    super(__pulumiType, name, args, opts);

    const parent = this;

    const fnArgs = normalizeFunction();
    const event = sst.output(args.event || {});
    normalizeTargets();
    const enabled = sst.output(args.enabled ?? true);
    ...
    function normalizeFunction() {
      ...
      return input ? sst.output(input) : undefined;
    }
        ...
        ...transform(
          ...
          fn
            ...
            : {
                ...
                roleArn: role!.arn,
                input: sst
                  .resolve([event, args.task!.containers])
                  .apply(([event, containers]) => {
