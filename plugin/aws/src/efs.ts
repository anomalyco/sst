...
export interface EfsArgs {
   ...
   * ```
   */
  throughput?: sst.Input<"provisioned" | "bursting" | "elastic">;
  /**
   ...
   * ```
   */
  performance?: sst.Input<"general-purpose" | "max-io">;
  /**
 ...
 */
export class Efs extends AWSComponent {
  private _fileSystem: sst.Output<efs.FileSystem>;
  private _accessPoint: sst.Output<efs.AccessPoint>;

  constructor(name: string, args: EfsArgs, opts?: sst.ComponentOptions) {
    super(__pulumiType, name, args, opts);

    if (args && "ref" in args) {
      const ref = args as unknown as EfsRef;
      this._fileSystem = sst.output(ref.fileSystem);
      this._accessPoint = sst.output(ref.accessPoint);
      return;
    }

    const parent = this;
    const vpc = normalizeVpc();
    const throughput = sst.output(args.throughput ?? "elastic");
    const performance = sst.output(args.performance ?? "general-purpose");
    ...
    const waited = mountTargets.apply((targets) =>
      sst.resolve(targets.map((target) => target.urn)).apply(() => ({
        fileSystem,
        accessPoint,
      })),
    );
