...
export type FunctionPermissionArgs = {
   ...
   * ```
   */
  resources: sst.Input<sst.Input<string>[]>;
};
...
interface FunctionUrlCorsArgs {
   ...
   * ```
   */
  allowCredentials?: sst.Input<boolean>;
  /**
   ...
   * ```
   */
  allowHeaders?: sst.Input<sst.Input<string>[]>;
  /**
   ...
   * ```
   */
  allowOrigins?: sst.Input<sst.Input<string>[]>;
  /**
   ...
   * ```
   */
  exposeHeaders?: sst.Input<sst.Input<string>[]>;
  /**
...
export interface FunctionArgs {
   ...
   * ```
   */
  permissions?: sst.Input<Prettify<FunctionPermissionArgs>[]>;
  /**
   ...
   * ```
   */
  streaming?: sst.Input<boolean>;
  /**
   * @internal
   */
  injections?: sst.Input<string[]>;
  /**
  ...
  url?: sst.Input<
    ...
    | {
         ...
         * ```
         */
        authorization?: sst.Input<"none" | "iam">;
        /**
         ...
         * ```
         */
        cors?: sst.Input<boolean | Prettify<FunctionUrlCorsArgs>>;
      }
  >;
  /**
   ...
   * ```
   */
  layers?: sst.Input<sst.Input<string>[]>;
  /**
...
export class Function extends sst.Component implements sst.Linkable {
  ...
  private logGroup: sst.Output<cloudwatch.LogGroup | undefined>;
  private urlEndpoint: sst.Output<string | undefined>;
  ...
  constructor(name: string, args: FunctionArgs, opts?: sst.ComponentOptions) {
    ...
    const isContainer = sst
      .resolve([args.python, dev])
      .apply(([python, dev]) => !dev && (python?.container ?? false));
    const partition = getPartitionOutput({}, opts).partition;
    const region = getRegionOutput({}, opts).name;
    const bootstrapData = region.apply((region) => bootstrap.forRegion(region));
    const injections = normalizeInjections();
    const runtime = sst.output(args.runtime ?? "nodejs20.x");
    const timeout = normalizeTimeout();
    const memory = normalizeMemory();
    const storage = sst.output(args.storage).apply((v) => v ?? "512 MB");
    const architecture = sst
      .output(args.architecture)
      .apply((v) => v ?? "x86_64");
    ...
    const policies = sst.output(args.policies ?? []);
    ...
    const buildInput = sst.output({
      ...
      copyFiles,
      properties: sst
        .output({ nodejs: args.nodejs, python: args.python })
        .apply((val) => ({
    ...
    function normalizeDev() {
      return sst
        .resolve([args.dev, args.live])
        .apply(([d, l]) => sst.dev && d !== false && l !== false);
    }

    function normalizeInjections() {
      return sst
        .output(args.injections)
        .apply((injections) => injections ?? []);
    }
    ...
    function normalizeMemory() {
      return sst.output(args.memory).apply((memory) => memory ?? "1024 MB");
    }
    ...
    function normalizeEnvironment() {
      return sst
        ...
        .apply(async ([environment, dev, bootstrap, key, links]) => {
          const result = environment ?? {};
          result.SST_RESOURCE_App = JSON.stringify({
            name: sst.app.name,
            stage: sst.app.stage,
          });
          for (const linkable of links || []) {
          ...
          if (dev) {
            ...
            result.SST_APP = sst.app.name;
            result.SST_STAGE = sst.app.stage;
    ...
    function normalizeStreaming() {
      return sst
        .output(args.streaming)
        .apply((streaming) => streaming ?? false);
    }
    ...
    function normalizeVolume() {
      ...
      return sst.output(args.volume).apply((volume) => ({
        efs:
          volume.efs instanceof Efs
            ? volume.efs.nodes.accessPoint.arn
            : sst.output(volume.efs),
        path: volume.path ?? "/mnt/efs",
      }));
    }

    function normalizeUrl() {
      return sst.output(args.url).apply((url) => {
    ...
    function buildHandler() {
      return sst
        .resolve([runtime, dev, isContainer])
        .apply(async ([runtime, dev, isContainer]) => {
          if (dev) {
            return {
              handler: "bootstrap",
    ...
    function createRole() {
      if (args.role) {
        return iam.Role.get(
          `${name}Role`,
          sst.output(args.role).apply(arn.parseRole).roleName,
          {},
          { parent },
        );
      }
      ...
      const policy = sst
        .resolve([args.permissions || [], linkPermissions, dev])
        .apply(([argsPermissions, linkPermissions, dev]) =>
              ...
              ...(dev
                ? [
                    ...
                    },
                    {
                      ...
                      actions: ["s3:*"],
                      resources: [
                        sst.interpolate`arn:${partition}:s3:::${bootstrapData.asset}`,
                        sst.interpolate`arn:${partition}:s3:::${bootstrapData.asset}/*`,
                      ],
                    },
                  ]
      ...
      return new iam.Role(
        ...transform(
          ...
          `${name}Role`,
          {
                  ...
                  statements: [
                    {
                      ...
                      principals: [
                        ...
                        },
                        {
                          type: "AWS",
                          identifiers: [
                            sst.interpolate`arn:${partition}:iam::${
            ...
            managedPolicyArns: sst
              .resolve([logging, policies])
              .apply(([logging, policies]) => [
                ...policies,
                ...(logging
                  ? [
                      sst.interpolate`arn:${partition}:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole`,
                    ]
                  : []),
                ...(vpc
                  ? [
                      sst.interpolate`arn:${partition}:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole`,
                    ]
                  : []),
              ]),
          },
    ...
    function createImageAsset() {
      ...
      return sst.resolve([isContainer, dev, bundle]).apply(
        ([
          isContainer,
          dev,
          ...
          return new Image(
            `${name}Image`,
            {
              ...
              cacheFrom: [
                {
                  registry: {
                    ref: sst.interpolate`${bootstrapData.assetEcrUrl}:${name}-cache`,
                  },
                },
              ],
              cacheTo: [
        ...
        .apply(
          async ([
            ...
            if (logGroupArn && sourcemaps) {
              ...
              for (const file of sourcemaps) {
                new s3.BucketObjectv2(
                  ...
                  {
                    key: sst.interpolate`sourcemap/${logGroupArn}/${hashValue}.${path.basename(
                      file,
                    )}`,
                    bucket: assetBucket,
                    source: sst.asset(file),
                  },
            ...
            return new s3.BucketObjectv2(
              `${name}Code`,
              {
                key: sst.interpolate`assets/${name}-code-${hashValue}.zip`,
                bucket: assetBucket,
                source: sst.asset(zipPath),
              },
              { parent },
            );
          },
      ...
      return logging.apply((logging) => {
        ...
        return new cloudwatch.LogGroup(
          ...transform(
            ...
            `${name}LogGroup`,
            {
              name: sst.interpolate`/aws/lambda/${
                args.name ?? sst.naming.physical(64, `${name}Function`)
              }`,
          ...
          ([
            ...
            const transformed = transform(
              ...
              `${name}Function`,
              {
                ...
                layers: args.layers,
                tags: args.tags,
                publish: sst.output(args.versioning).apply((v) => v ?? false),
                ...
                ...(isContainer
                  ? {
                      ...
                      imageConfig: {
                        commands: [
                          sst
                            .resolve([handler, runtime])
                            .apply(([handler, runtime]) => {
                ...
                ...(dev
                  ? {
                      description: transformed[1].description
                        ? sst
                            .output(transformed[1].description)
                            .apply((v) => `${v.substring(0, 240)} (live)`)
                        : "live",
    ...
    function createUrl() {
      return url.apply((url) => {
        if (url === undefined) return sst.output(undefined);
    ...
    function createProvisioned() {
      return sst
        .resolve([args.concurrency, fn.publish])
        .apply(([concurrency, publish]) => {
  ...
  static fromDefinition(
    name: string,
    definition: sst.Input<string | FunctionArgs>,
        ...
        return new Function(
          ...transform(
            ...
            name,
            {
              ...
              ...override,
              permissions: sst
                .resolve([definition.permissions, override?.permissions])
