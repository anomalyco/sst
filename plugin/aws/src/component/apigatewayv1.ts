...
export interface ApiGatewayV1Args {
   ...
   * ```
   */
  domain?: sst.Input<string | Prettify<ApiGatewayV1DomainArgs>>;
  /**
  ...
  endpoint?: sst.Input<{
     ...
     */
    vpcEndpointIds?: sst.Input<sst.Input<string>[]>;
  }>;
  /**
...
export interface ApiGatewayV1AuthorizerArgs {
   ...
   * ```
   */
  tokenFunction?: sst.Input<string | FunctionArgs>;
  /**
   ...
   * ```
   */
  requestFunction?: sst.Input<string | FunctionArgs>;
  /**
   ...
   * ```
   */
  userPools?: sst.Input<sst.Input<string>[]>;
  /**
   ...
   * ```
   */
  identitySource?: sst.Input<string>;
  /**
   * [Transform](/docs/components#transform) how this component creates its underlying
   * resources.
   */
...
export interface ApiGatewayV1UsagePlanArgs {
  ...
  quota?: sst.Input<{
     ...
     * time.
     */
    limit: sst.Input<number>;
    /**
     * The time period for which the quota applies.
     */
    period: sst.Input<"day" | "week" | "month">;
    /**
 ...
 */
export class ApiGatewayV1 extends sst.Component implements sst.Linkable {
  private constructorName: string;
  private constructorArgs: ApiGatewayV1Args;
  private constructorOpts: sst.ComponentOptions;
  private api: apigateway.RestApi;
  private apigDomain?: sst.Output<apigateway.DomainName>;
  private apiMapping?: sst.Output<apigateway.BasePathMapping>;
  private region: sst.Output<string>;
  private resources: Record<string, sst.Output<string>> = {};
  ...
  private endpointType: sst.Output<"EDGE" | "REGIONAL" | "PRIVATE">;
  ...
  constructor(
    ...
    function normalizeEndpoint() {
      return sst.output(args.endpoint).apply((endpoint) => {
  ...
  public get url() {
    return this.apigDomain && this.apiMapping
      ? sst
          .resolve([this.apigDomain.domainName, this.apiMapping.basePath])
          .apply(([domain, key]) =>
            key ? `https://${domain}/${key}/` : `https://${domain}`,
          )
      : sst.interpolate`https://${this.api.id}.execute-api.${this.region}.amazonaws.com/${sst.app.stage}/`;
  }
  ...
  public route(
    route: string,
    handler: sst.Input<string | FunctionArgs | FunctionArn>,
  ...
  private buildRouteId(method: string, path: string) {
    const suffix = sst.naming.logical(
      sst.naming.hashStringToPrettyString([outputId, method, path].join(""), 6),
    );
  ...
  public addAuthorizer(args: ApiGatewayV1AuthorizerArgs) {
    ...
    const nameSuffix = sst.naming.logical(args.name);
  ...
  public deploy() {
    ...
    function normalizeAccessLog() {
      return sst.output(args.accessLog).apply((accessLog) => ({
        ...accessLog,
    ...
    function createCorsRoutes() {
      ...
      return sst
        .resolve([args.cors, resourceIds])
        .apply(([cors, resourceIds]) => {
    ...
    function createDeployment() {
      const resources = sst
        .resolve([corsRoutes, corsResponses])
        .apply(([corsRoutes, corsResponses]) =>
          [
            api,
    ...
    function createLogGroup() {
      return new cloudwatch.LogGroup(
        ...transform(
          ...
          `${name}AccessLog`,
          {
            name: `/aws/vendedlogs/apis/${sst.naming.physical(64, name)}`,
    ...
    function createSsl() {
      ...
      return sst
        .resolve([domain, endpointType, region])
        .apply(([domain, endpointType, region]) => {
          if (domain.cert) return sst.output(domain.cert);
          if (domain.nameId) return sst.output(undefined);
                    ...
                    ...(endpointType === "REGIONAL"
                      ? {
                          regionalCertificateArn:
                            certificateArn as sst.Output<string>,
                        }
                      : {
                          certificateArn: certificateArn as sst.Output<string>,
                        }),
                  },
                  { parent },
                ),
