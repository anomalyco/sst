 ...
 */
export class ApiGatewayWebSocketRoute extends AWSComponent {
  private readonly fn: FunctionBuilder;
  private readonly permission: lambda.Permission;
  private readonly apiRoute: sst.Output<apigatewayv2.Route>;
  ...
  constructor(name: string, args: Args, opts?: ComponentResourceOptions) {
    ...
    const route = sst.output(args.route);
    ...
    function createFunction() {
      return functionBuilder(
        ...
        args.handler,
        {
          description: sst.interpolate`${api.name} route ${route}`,
        },
    ...
    function createPermission() {
      return new lambda.Permission(
        `${name}Permissions`,
        {
          ...
          sourceArn: sst.interpolate`${api.executionArn}/*`,
        },
        { parent: self },
      );
    }
          ...
          new apigatewayv2.Route(
            ...transform(
              ...
              `${name}Route`,
              {
                apiId: api.id,
                routeKey: route,
                target: sst.interpolate`integrations/${integration.id}`,
                ...authArgs,
              },
              { parent: self },
            ),
