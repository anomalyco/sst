...
interface InlineRouterBucketRouteArgs extends InlineBaseRouteArgs {
   ...
   * ```
   */
  bucket?: sst.Input<Bucket | string>;
  /**
   * Rewrite the request path.
   *
   * @example
...
interface RouteArgs {
   ...
   * ```
   */
  connectionAttempts?: sst.Input<number>;
  /**
   ...
   * ```
   */
  connectionTimeout?: sst.Input<DurationSeconds>;
}
...
export interface RouterUrlRouteArgs extends RouteArgs {
   ...
   * ```
   */
  readTimeout?: sst.Input<DurationSeconds>;
  /**
   ...
   * ```
   */
  keepAliveTimeout?: sst.Input<DurationSeconds>;
}
...
export interface RouterArgs {
  ...
  routes?: sst.Input<
    Record<
      string,
      sst.Input<string | InlineUrlRouteArgs | InlineRouterBucketRouteArgs>
    >
  >;
  /**
   * Configure CloudFront Functions to customize the behavior of HTTP requests and responses at the edge.
  ...
  invalidation?: sst.Input<
    ...
    | {
         ...
         */
        paths?: sst.Input<sst.Input<string>[]>;
      }
  >;
 ...
 *   entries.
 */
export class Router extends AWSComponent implements sst.Linkable {
  private constructorName: string;
  private constructorOpts: sst.ComponentOptions;
  private cdn: sst.Output<Cdn>;
  private kvStoreArn?: sst.Output<string>;
  private kvNamespace?: sst.Output<string>;
  private hasInlineRoutes: sst.Output<boolean>;
  ...
  constructor(
    ...
    if (hasInlineRoutes) {
      ...
      cdn = sst.output(r.distribution);
      kvStoreArn = r.kvStoreArn;
      kvNamespace = sst.output(r.kvNamespace);
    }

    this.cdn = cdn;
    this.kvStoreArn = kvStoreArn;
    this.kvNamespace = kvNamespace;
    this.hasInlineRoutes = sst.output(hasInlineRoutes);
    registerOutputs();
    ...
    function handleInlineRoutes() {
      ...
      function normalizeRoutes() {
        return sst.output(args.routes!).apply((routes) => {
      ...
      function createCfRequestFunction(
        ...
        return new cloudfront.Function(
          `${name}CloudfrontFunction${sst.naming.hashStringToPrettyString(
            path,
            8,
          )}`,
          {
...
event.request.uri = event.request.uri.replace(re, "${rewrite.to}");`
  ...
  return event.request;
      ...
      function createCfResponseFunction(
        ...
        return new cloudfront.Function(
          `${name}CloudfrontFunctionResponse${sst.naming.hashStringToPrettyString(
            path,
            8,
          )}`,
          {
      ...
      function createOriginAccessControl() {
        defaultOac =
          ...
          new OriginAccessControl(
            ...
            { name: sst.naming.physical(64, name) },
    ...
    function handleLazyRoutes() {
      ...
      function createRequestKvStore() {
        return sst.output(args.edge).apply((edge) => {
          ...
          if (userKvStore) return sst.output(userKvStore);
  ...
  async function getRoutes() {
    ...
    try {
      ...
      if (routes.parts) {
        ...
        const chunks = await Promise.sst.all(chunkPromises);
...
async function handler(event) {
          ...
          ...transform(
            ...
            `${name}Cdn`,
            {
              ...
              defaultCacheBehavior: {
                ...
                functionAssociations: sst
                  .resolve([requestFunction, responseFunction])
                  .apply(([reqFn, resFn]) => [
  ...
  public get url() {
    return sst
      .resolve([this.cdn.domainUrl, this.cdn.url])
      .apply(([domainUrl, url]) => domainUrl ?? url);
  }
  ...
  public route(
    ...
    url: sst.Input<string>,
    args?: sst.Input<RouterUrlRouteArgs>,
  ) {
    sst
      .resolve([pattern, args, this.hasInlineRoutes])
      .apply(([pattern, args, hasInlineRoutes]) => {
        if (hasInlineRoutes)
          throw new VisibleError(
  ...
  public routeBucket(
    ...
    args?: sst.Input<RouterBucketRouteArgs>,
  ) {
    sst
      .resolve([pattern, args, this.hasInlineRoutes])
      .apply(([pattern, args, hasInlineRoutes]) => {
   ...
   */
  public routeSite(pattern: sst.Input<string>, site: any) {
    throw new VisibleError(
...
export type RouterRouteArgsDeprecated = {
  router: sst.Input<Router>;
  domain?: sst.Input<string>;
  path?: sst.Input<string>;
};

export function normalizeRouteArgs(
  route?: sst.Input<RouterRouteArgs>,
  routeDeprecated?: sst.Input<RouterRouteArgsDeprecated>,
) {
  if (!route && !routeDeprecated) return undefined;

  return sst
    .resolve([route, routeDeprecated])
    .apply(([route, routeDeprecated]) => {
      const v = route
        ? route
