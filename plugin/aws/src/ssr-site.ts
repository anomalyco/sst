...
export type Plan = {
  ...
  // when routing to S3.
  base?: string;
  server?: sst.Unwrap<FunctionArgs>;
  imageOptimizer?: {
    function: sst.Unwrap<FunctionArgs>;
    prefix: string;
  };
  assets: {
    from: string;
...
export interface SsrSiteArgs extends BaseSsrSiteArgs {
  ...
  invalidation?: sst.Input<
    ...
    | {
         ...
         */
        paths?: sst.Input<"all" | "versioned" | string[]>;
      }
  >;
  /**
  ...
  server?: {
     ...
     * ```
     */
    runtime?: sst.Input<"nodejs18.x" | "nodejs20.x" | "nodejs22.x">;
    /**
...
export abstract class SsrSite extends AWSComponent implements sst.Linkable {
  private cdn?: Cdn;
  private bucket?: Bucket;
  private server?: sst.Output<Function>;
  private devUrl?: sst.Output<string>;
  private prodUrl?: sst.Output<string | undefined>;

  protected abstract normalizeBuildCommand(
    args: SsrSiteArgs,
  ): sst.Output<string> | void;
  ...
  constructor(
    ...
    const purge = sst
      .output(args.assets)
      .apply((assets) => assets?.purge ?? false);
    ...
    const timeout = sst
      .resolve([serverTimeout, plan.server])
      .apply(
...
async function handler(event) {
  ...
  return event.response;
          ...
          `${name}Cdn`,
          {
            ...
            defaultCacheBehavior: {
              ...
              functionAssociations: sst
                .resolve([createRequestFunction(), createResponseFunction()])
                .apply(([reqFn, resFn]) => [
    ...
    function normalizeEdge() {
      return sst
        .output([args.edge, args.server?.edge])
        .apply(([edge, serverEdge]) => {
          if (serverEdge)
            throw new VisibleError(
    ...
    function normalizeServerTimeout() {
      return sst.output(args.server?.timeout).apply((v) => {
        if (!v) return v;
    ...
    function validatePlan(plan: sst.Output<Plan>) {
      return sst.resolve([plan, route]).apply(([plan, route]) => {
        if (plan.base) {
          // starts with /
    ...
    function createServers() {
      return sst
        .resolve([regions, plan.server])
        .apply(([regions, planServer]) => {
          ...
          return regions.map((region) => {
            ...
            const server = new Function(
              ...transform(
                args.transform?.server,
                `${name}Server${sst.naming.logical(region)}`,
                {
                  ...planServer,
                  description: planServer.description ?? `${name} server`,
                  runtime: sst
                    .output(args.server?.runtime)
                    .apply((v) => v ?? planServer.runtime ?? "nodejs20.x"),
                  timeout,
                  memory: sst
                    .output(args.server?.memory)
                    .apply((v) => v ?? planServer.memory ?? "1024 MB"),
                  architecture: sst
                    .output(args.server?.architecture)
                    .apply((v) => v ?? planServer.architecture ?? "x86_64"),
                  vpc: args.vpc,
                  nodejs: {
                    format: "esm" as const,
            ...
            if (args.warm) {
              // Create cron job
              const cron = new Cron(
                `${name}Warmer${sst.naming.logical(region)}`,
                {
              ...
              new lambda.Invocation(
                `${name}Prewarm${sst.naming.logical(region)}`,
                {
    ...
    function createImageOptimizer() {
      return sst.output(plan.imageOptimizer).apply((imageOptimizer) => {
    ...
    function uploadAssets() {
      return sst
        .resolve([args.assets, route, plan, outputPath])
        .apply(async ([assets, route, plan, outputPath]) => {
              ...
              .apply(([servers, imageOptimizerUrl]) => {
                ...
                plan.assets.forEach((copy) => {
                  fs.readdirSync(path.join(outputPath, copy.from), {
                    ...
                    fs.readdirSync(
                      path.join(outputPath, copy.from, item.name),
                      {
                        withFileTypes: true,
                      },
                    ).forEach((subItem) => {
                ...
                kvEntries["metadata"] = JSON.stringify({
                  ...
                  servers: servers.map((s) => [
                    new URL(s.url).host,
                    supportedRegions[s.region as keyof typeof supportedRegions]
                      .lat,
                    supportedRegions[s.region as keyof typeof supportedRegions]
                      .lon,
                  ]),
    ...
    function createInvalidation() {
      sst
        .resolve([args.invalidation, outputPath, plan])
        .apply(([invalidationRaw, outputPath, plan]) => {
  ...
  public get url() {
    return sst
      .resolve([this.prodUrl, this.devUrl])
      .apply(([prodUrl, devUrl]) => (prodUrl ?? devUrl)!);
  }
