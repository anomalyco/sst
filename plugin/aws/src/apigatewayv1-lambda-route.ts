import { lambda, apigateway } from "@pulumi/aws";
...
export interface Args extends ApiGatewayV1BaseRouteArgs {
   ...
   * The route function.
   */
  handler: sst.Input<string | FunctionArgs>;
  /**
   * @internal
   */
 ...
 */
export class ApiGatewayV1LambdaRoute extends sst.Component {
  private readonly fn: FunctionBuilder;
  private readonly permission: lambda.Permission;
  private readonly method: sst.Output<apigateway.Method>;
  ...
  constructor(name: string, args: Args, opts?: sst.ComponentOptions) {
    ...
    function createFunction() {
      ...
      return functionBuilder(
        ...
        args.handler,
        {
          description: sst.interpolate`${api.name} route ${method} ${path}`,
        },
