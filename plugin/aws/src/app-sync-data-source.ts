 ...
 */
export class AppSyncDataSource extends sst.Component {
  ...
  constructor(name: string, args: DataSourceArgs, opts?: sst.ComponentOptions) {
    super(__pulumiType, name, args, opts);

    const self = this;

    const apiId = sst.output(args.apiId);
        ...
        ...transform(
          ...
          `${name}DataSource`,
          {
            ...
            dynamodbConfig: args.dynamodb
              ? {
                  tableName: sst
                    .output(args.dynamodb)
                    .apply((item) => arn.parseDynamo(item).tableName),
                }
              : undefined,
            ...
            relationalDatabaseConfig: args.rds
              ? {
                  httpEndpointConfig: {
                    dbClusterIdentifier: sst.output(args.rds).cluster,
                    awsSecretStoreArn: sst.output(args.rds).credentials,
                  },
                }
              : undefined,
          },
