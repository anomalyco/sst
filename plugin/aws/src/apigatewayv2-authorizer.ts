 ...
 */
export class ApiGatewayV2Authorizer extends sst.Component {
  private readonly authorizer: apigatewayv2.Authorizer;

  constructor(name: string, args: AuthorizerArgs, opts?: sst.ComponentOptions) {
    super(__pulumiType, name, args, opts);

    const self = this;

    const api = sst.output(args.api);
    const lamb = args.lambda && sst.output(args.lambda);
    const jwt = args.jwt && sst.output(args.jwt);
