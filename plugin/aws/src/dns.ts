...
export function dns(args: DnsArgs = {}) {
  ...
  function _createRecord(
    ...
    return sst.output(partial).apply((partial) => {
      const nameSuffix = sst.naming.logical(partial.name);
      ...
      function lookupZone() {
        if (args.zone) {
          return sst.output(args.zone).apply(async (zoneId) => {
