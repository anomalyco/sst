...
export interface AuroraArgs {
   ...
   * ```
   */
  engine: sst.Input<"postgres" | "mysql">;
  /**
  ...
  scaling?: sst.Input<{
     ...
     * ```
     */
    pauseAfter?: sst.Input<DurationHours>;
  }>;
  /**
 ...
 * for more details.
 */
export class Aurora extends sst.Component implements sst.Linkable {
  ...
  constructor(name: string, args: AuroraArgs, opts?: sst.ComponentOptions) {
    ...
    const engine = sst.output(args.engine);
    const version = sst
      .resolve([args.version, engine])
      .apply(
        ([version, engine]) =>
          version ?? { postgres: "16.4", mysql: "3.08.0" }[engine],
      );
    const username = sst
      .resolve([args.username, engine])
      .apply(
        ([username, engine]) =>
    ...
    const dataApi = sst.output(args.dataApi).apply((v) => v ?? false);
    ...
    function registerDev() {
      ...
      new DevCommand(`${name}Dev`, {
        ...
        environment: {
          SST_DEV_COMMAND_MESSAGE: sst.interpolate`Make sure your local database is using:
...
Listening on "${dev.host}:${dev.port}"...`,
    ...
    function createSecret() {
      ...
      new secretsmanager.SecretVersion(
        ...
        {
          secretId: secret.id,
          secretString: sst.json.stringify({ username, password }),
        },
        { parent: self },
      );
    ...
    function createProxy() {
      return sst.resolve([args.proxy]).apply(([proxy]) => {
        if (!proxy) return;
  ...
  public get host() {
    ...
    return sst
      .resolve([this.cluster!.endpoint, this.proxy!])
      .apply(
        ([endpoint, proxy]) =>
      ...
      properties: {
        ...
        reader: this.dev?.enabled
          ...
          : sst
              .resolve([this.cluster!.readerEndpoint, this.proxy!])
              .apply(([endpoint, proxy]) => {
                if (proxy) return sst.output(undefined);
                return sst.output(endpoint.split(":")[0]);
              }),
      },
