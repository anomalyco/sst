...
export interface VpcArgs {
   ...
   * ```
   */
  az?: sst.Input<number | sst.Input<string>[]>;
  /**
...
export class Vpc extends AWSComponent implements sst.Linkable {
  ...
  private natInstances: sst.Output<ec2.Instance[]>;
  ...
  private bastionInstance: sst.Output<ec2.Instance | undefined>;
  private cloudmapNamespace: servicediscovery.PrivateDnsNamespace;
  private privateKeyValue: sst.Output<string | undefined>;
  ...
  constructor(name: string, args: VpcArgs = {}, opts?: sst.ComponentOptions) {
    ...
    this.bastionInstance = sst.output(bastionInstance);
    this.cloudmapNamespace = cloudmapNamespace;
    this.privateKeyValue = sst.output(privateKeyValue);
    registerOutputs();
    ...
    function reference() {
      ...
      const natGateways = publicSubnets.apply((subnets) => {
        const natGatewayIds = subnets.map((subnet) =>
          ec2
        ...
        return sst.output(natGatewayIds).apply((ids) =>
          ids
            .filter((id) => id)
            .map((id, i) =>
      ...
      const zone = sst.output(vpcId).apply((vpcId) =>
        route53.getZone(
          {
            name: "sst",
            privateZone: true,
      ...
      const privateKeyValue = bastionInstance.apply((v) => {
        ...
        const param = ssm.Parameter.get(
          ...
          sst.interpolate`/sst/vpc/${vpcId}/private-key-value`,
          undefined,
          { parent: self },
        );
        return param.value;
    ...
    function normalizeAz() {
      return sst.output(args.az).apply((az) => {
        if (Array.isArray(az)) return sst.output(az);
    ...
    function normalizeNat() {
      return sst.resolve([args.nat, zones]).apply(([nat, zones]) => {
        if (nat === "managed") {
      ...
      return new ec2.Vpc(
        ...transform(
          ...
          `${name}Vpc`,
          {
            ...
            tags: {
              Name: `${sst.app.name}-${sst.app.stage}-${name} VPC`,
    ...
    function createKeyPair() {
      const ret = sst.output(args.bastion).apply((bastion) => {
        if (!bastion) return {};
        ...
        new ssm.Parameter(
          `${name}PrivateKeyValue`,
          {
            name: sst.interpolate`/sst/vpc/${vpc.id}/private-key-value`,
      ...
      return {
        keyPair: sst.output(ret.keyPair),
        privateKeyValue: sst.output(ret.privateKeyValue),
      };
    }
    ...
    function createElasticIps() {
      return sst.resolve([nat, publicSubnets]).apply(([nat, subnets]) => {
        if (!nat) return [];
        if (nat?.ip) return [];
    ...
    function createNatGateways() {
      return sst
        .resolve([nat, publicSubnets, elasticIps])
        .apply(([nat, subnets, elasticIps]) => {
    ...
    function createNatInstances() {
      return nat.apply((nat) => {
        if (nat?.type !== "ec2") return sst.output([]);
        ...
        const role = new iam.Role(
          `${name}NatInstanceRole`,
          {
            ...
            managedPolicyArns: [
              sst.interpolate`arn:${partition}:iam::aws:policy/AmazonSSMManagedInstanceCore`,
            ],
          },
          { parent: self },
        );
        ...
        return sst
          .resolve([zones, publicSubnets, elasticIps, keyPair, args.bastion])
          ...
          const routeTable = new ec2.RouteTable(
            ...transform(
              ...
              `${name}PrivateRouteTable${i + 1}`,
              {
                vpcId: vpc.id,
                routes: sst
                  .resolve([natGateways, natInstances])
    ...
    function createBastion() {
      return sst
        .resolve([args.bastion, natInstances, keyPair])
        .apply(([bastion, natInstances, keyPair]) => {
  ...
  public get securityGroups() {
    return sst.output(this.securityGroup).apply((v) => [v.id]);
  }
