...
interface TaskBaseArgs extends StateArgs {
   ...
   * ```
   */
  integration?: sst.Input<"response" | "sync" | "token">;
  /**
   ...
   */
  timeout?: sst.Input<JSONata | Duration>;
}
...
export interface TaskArgs extends TaskBaseArgs {
   ...
   * ```
   */
  arguments?: sst.Input<Record<string, sst.Input<any>>>;
  /**
...
export class Task extends State implements Nextable, Failable {
  private resource: sst.Output<string>;

  constructor(protected args: TaskArgs) {
    super(args);

    const integration = sst.output(this.args.integration ?? "response");
    this.resource = sst
      .resolve([this.args.resource, integration])
      .apply(([resource, integration]) => {
    ...
    return {
      ...
      Timeout: this.args.timeout
        ? sst
            ...
            .apply((t) => (isJSONata(t) ? t : toSeconds(t as Duration)))
        : undefined,
...
export interface LambdaInvokeArgs extends TaskBaseArgs {
   ...
   */
  function: Function | sst.Input<string | FunctionArgs | FunctionArn>;
  /**
   ...
   * ```
   */
  payload?: Record<string, sst.Input<unknown>>;
}
...
export interface SnsPublishArgs extends TaskBaseArgs {
   ...
   * ```
   */
  messageAttributes?: sst.Input<Record<string, sst.Input<string>>>;
  /**
...
export interface SqsSendMessageArgs extends TaskBaseArgs {
   ...
   */
  messageBody: sst.Input<string | Record<string, sst.Input<unknown>>>;
  /**
...
export interface EventBridgePutEventsArgs extends TaskBaseArgs {
  ...
  events: {
     ...
     * ```
     */
    detail?: sst.Input<Record<string, sst.Input<unknown>>>;
  }[];
}
