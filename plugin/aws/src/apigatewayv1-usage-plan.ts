 ...
 */
export class ApiGatewayV1UsagePlan extends sst.Component {
  ...
  constructor(
    ...
    this.plan = new apigateway.UsagePlan(
      `${name}UsagePlan`,
      {
        ...
        quotaSettings:
          args.quota &&
          sst.output(args.quota).apply((quota) => ({
            limit: quota.limit,
        ...
        throttleSettings:
          args.throttle &&
          sst.output(args.throttle).apply((throttle) => ({
