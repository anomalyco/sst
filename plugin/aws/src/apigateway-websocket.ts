...
export interface ApiGatewayWebSocketAuthorizerArgs {
  ...
  lambda?: sst.Input<{
     ...
     * ```
     */
    function: sst.Input<string | FunctionArgs>;
    /**
 ...
 * ```
 */
export class ApiGatewayWebSocket extends sst.Component implements sst.Linkable {
  ...
  constructor(
    ...
    function createDomainName() {
      ...
      return sst
        .resolve([domain, certificateArn])
        .apply(([domain, certificateArn]) => {
  ...
  public get url() {
    ...
    return this.apigDomain && this.apiMapping
      ...
      : sst.interpolate`${this.api.apiEndpoint}/${this.stage.name}`;
  }
  ...
  public get managementEndpoint() {
    ...
    return this.api.apiEndpoint.apply(
      (endpoint) =>
        sst.interpolate`${endpoint.replace("wss", "https")}/${this.stage.name}`,
    );
  }
  ...
  public route(
    ...
    const suffix = sst.naming.logical(
      ["$connect", "$disconnect", "$default"].includes(route)
        ? route
        : sst.naming.hashStringToPrettyString(`${outputId}${route}`, 6),
    );
    ...
    return {
      ...
      include: [
        permission({
          ...
          resources: [
            sst.interpolate`${this.api.executionArn}/*/*/@connections/*`,
          ],
        }),
      ],
    };
