...
export function buildApp(
  ...
  return resolve([
    ...
    function resolveBuildCommand() {
      ...
      if (!userCommand) {
        if (!fs.existsSync(join(sitePath, "package.json"))) {
          throw new VisibleError(`No package.json found at "${sitePath}".`);
        }
        const packageJson = JSON.parse(
          fs.readFileSync(join(sitePath, "package.json")).toString(),
        );
      ...
      if (
        fs.existsSync(join(sitePath, "yarn.lock")) ||
        fs.existsSync(join(path.root, "yarn.lock"))
      )
        return "yarn run build";
      if (
        fs.existsSync(join(sitePath, "pnpm-lock.yaml")) ||
        fs.existsSync(join(path.root, "pnpm-lock.yaml"))
      )
        return "pnpm run build";
      if (
        fs.existsSync(join(sitePath, "bun.lockb")) ||
        fs.existsSync(join(path.root, "bun.lockb")) ||
        fs.existsSync(join(sitePath, "bun.lock")) ||
        fs.existsSync(join(path.root, "bun.lock"))
      )
