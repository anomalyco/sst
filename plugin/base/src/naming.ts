...
export namespace naming {
  ...
  export function logical(name: string) {
    name = name.replace(/[^a-zA-Z0-9]/g, "");
    return name.charAt(0).toUpperCase() + name.slice(1);
  }

  export function physical(max: number, name: string, suffix: string = "") {
    ...
    const main = prefix(max - 9 - suffix.length, name);
  ...
  export function prefix(max: number, name: string) {
    // This function does the following:
    // - Removes all non-alphanumeric characters
    // - Prefixes the name with the app name and stage
    // - Truncates the name if it's too long
