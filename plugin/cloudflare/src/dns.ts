...
export function dns(args: DnsArgs = {}) {
  ...
  function handleCreate(
    ...
    return sst.output(record).apply((record) => {
      const zone = lookupZone(namePrefix, record.type, record.name, opts);
      const proxy = sst
        .output(args.proxy)
        .apply((proxy) => (proxy && record.isAlias) ?? false);
      const nameSuffix = sst.naming.logical(record.name);
      ...
      return new cloudflare.Record(
        ...transform(
          ...
          `${namePrefix}${record.type}Record${nameSuffix}`,
          {
            ...
            ttl: sst.output(proxy).apply((proxy) => (proxy ? 1 : 60)),
