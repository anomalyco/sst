...
Use the `awslambda.streamifyResponse` function to wrap your handler. The `awslambda` global is provided by the Lambda execution environment at runtime, and SST provides it automatically during `sst dev` as well. For TypeScript types, importing from `@types/aws-lambda` will augment the global namespace.
