...
If you want to pass links to compute not managed by SST, like a native ECS task definition, use `Linkable.env()`. It converts your linked resources into `SST_RESOURCE_*` environment variables so `Resource.XXX` works at runtime.
