...
export async function getStaticPaths() {
  ...
  // Filter to only docs that start with "blog/"
  const filtered = docs.filter((doc) => {
    return doc.slug.startsWith("blog/") && !doc.slug.endsWith("/index");
  });

  return filtered.map((doc) => ({
    // Remove "blog/" prefix for the param
    params: { slug: doc.slug.replace(/^blog\//, '') },
  }));
}

export const GET: APIRoute = async ({ params }) => {
  // Add "blog/" prefix back to get the actual slug
